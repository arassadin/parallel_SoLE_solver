...
void conjugateGradientSolver::setAccuracy(double _accuracy)
{
	accuracy=_accuracy;
}
...
void conjugateGradientSolver::freeMemory()
{
	for(int i=0; i<systemSize; i++)
		delete[] matrix[i];
	delete matrix;
...
bool conjugateGradientSolver::stopCondition(double** _discrepancy)
{
	bool status(true);
	for(int i=0; i<systemSize; i++)
